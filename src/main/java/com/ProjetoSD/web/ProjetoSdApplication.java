...
public class ProjetoSdApplication {
    ...
    //TOP STORIES, get request on server, se tiver os termos de pesquisa da filter das top stories
    // fazer mm na rota para ver se é preciso fazer o request com as hacker news ou n
	/*
		Fazer uma check box a dizer que quero indexar as top hacker news, se sim, fazer o request na rota
		se n tiver checked fazer o request no search model normalmente
	 */
    // vai haver um botão que indexa os meus termos de pesquisa no request das hacker news

	/*
	Indexar as top stories
{
  "by" : "dhouston",
  "descendants" : 71,
  "id" : 8863,
...
Ver no text se têm os termos que pretendo e indexar esses termos, devolvendo o link que tambem está no json
se n houver url cagar nisso
	 */

	/*
	Indexar as user stories
	Há outra rota diferente para utilizadores:
	{
	...
	o id do user é o jl
	https://hacker-news.firebaseio.com/v0/user/<nomedouser>.json?print=pretty

	ex: https://hacker-news.firebaseio.com/v0/user/test.json?print=pretty -> User test

	Fazer um request para cada um dos ids e indexar todas as stories que têm esse url, fazer X pedidos para todos os ids submitted
	 */


    // SO PRECISAMOS DE USAR JAVASCRIPT NO PROJETO PARA ISTO:
    // todo no final
    // todo na pagina de administração temos de criar um socket para receber os pedidos de indexação em tempo real
    // todo este server tambem tem de ter uma ligação de socket
    // o web server liga-se ao server model por rmi, fazemos um while true e vamos recebendo pedidos de indexação
    // depois mando a info do RMI p/ sockets
